...
async function addShop() {
 ...
 try {
   ...
   const response = await fetch(API_URL, {
     ...
     body: JSON.stringify({ name: name, comment: comment }), // ★【修正箇所①】comment も一緒にJSONにして送る
   });
   if (!response.ok) {
   ...
   if (commentInput) commentInput.value = ""; // ★【修正箇所①】メモの入力欄も空に戻す
...
function renderShops(shops) {
 ...
 shops.forEach((shop) => {
   ...
   // ★【修正箇所②】メモを表示するための文字（span）を新しく作る！
   if (shop.comment) {
     const commentSpan = document.createElement("span");
     commentSpan.className = "shop-comment";
     commentSpan.style.marginLeft = "15px"; // 少し隙間をあける
     commentSpan.style.color = "#666";      // 文字色を少し薄くする
     commentSpan.style.fontSize = "0.9em";   // 文字を少し小さくする
     commentSpan.textContent = `(${shop.comment})`; // カッコで囲んで表示
